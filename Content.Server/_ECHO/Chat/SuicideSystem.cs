...
public sealed partial class SuicideSystem : EntitySystem
{
    ...
    public bool Suicide(EntityUid victim)
    {
        ...
        TryEnvironmentalSuicide(victim, ref suicideEvent);
        TryDamageableSuicide(victim, ref suicideEvent);
        // ECHO-Tweak-end
    ...
    /// </summary>
    private bool TryEnvironmentalSuicide(Entity<MobStateComponent?> victim, ref SuicideEvent args)
    {
    ...
    /// </summary>
    private bool TryDamageableSuicide(Entity<DamageableComponent?> victim, ref SuicideEvent args)
    {
        if (!Resolve(victim, ref victim.Comp, false))
            return false;
        ...
        {
            _suicide.ApplyLethalDamage((victim.Owner, victim.Comp), args.DamageSpecifier);
            args.Handled = true;
            return true;
        }

        args.DamageType ??= "Bloodloss";
        _suicide.ApplyLethalDamage((victim.Owner, victim.Comp), args.DamageType);
        args.Handled = true;
        return true;
    }
}
